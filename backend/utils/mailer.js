...
exports.sendOTP = async (to, otp, purpose = 'MFA') => {
  const isPasswordReset = purpose === 'Password Reset';
  const subject = isPasswordReset ? 'Password Reset Code' : 'Your MFA Verification Code';
  const title = isPasswordReset ? 'Password Reset' : 'Multi-Factor Authentication (MFA)';
  const instruction = isPasswordReset 
    ? 'Enter this code to reset your password.'
    : 'Enter this code in the login screen to complete your sign in.';
  
  await transporter.sendMail({
    from: `"WorkTrust Lite" <${process.env.EMAIL_USER}>`,
    to,
    subject,
    text: `Your ${purpose.toLowerCase()} verification code is: ${otp}`,
    html: `
      <div style="font-family: 'Segoe UI', Arial, sans-serif; background: #f6fffa; padding: 32px 24px; border-radius: 12px; max-width: 420px; margin: 0 auto; border: 1px solid #e0f2f1;">
        ...
        <p style="color: #222; font-size: 16px; text-align: center; margin-bottom: 24px;">Your <b>${title}</b> code is:</p>
        ...
        <p style="color: #444; font-size: 15px; text-align: center; margin-bottom: 16px;">${instruction}<br/>This code will expire in 5 minutes.</p>
